...
export default class UserController {
  ...
  static async editInfo(req, res): Promise<object> {

    let notValid = validate(req.body, validation.editInfo());
    if (notValid) return errRes(res, notValid);
  ...
  static async phone(req, res): Promise<object> {
    let notValid = validate(req.body, validation.phone());
    if (notValid) return errRes(res, notValid);
   ...
   user = await User.findOne({ where: { phone: req.body.phone } });
    ...
    if (! user )  return errRes(res, "phone is wrong");
  ...
  static async resetPass(req, res): Promise<object> {
    
    let notValid = validate(req.body, validation.resetPass());
    if (notValid) return errRes(res, notValid)
...
let newPasswrd : any;
    ...
    if (req.body.otp= user.otp)
     newPasswrd = await hashMyPassword(req.body.newPassword);
    user.password= req.body.newPasswrd
    await user.save();
    user.password= null
    return okRes(res, { data: { user } });
  }
